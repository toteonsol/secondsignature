...
async function gatherContext(vault, id, p) {
  ...
  const mine = decisions.filter((d) => d.vault.toLowerCase() === vault.toLowerCase());
  const priorDecisions = mine.slice(-5);
  // The guardian's memory of this owner: what normal looks like for this vault.
  const approvedAmts = mine.filter((d) => d.approved && d.ctx).map((d) => Number(d.ctx.amountMON)).sort((x, y) => x - y);
  const destCounts = {};
  for (const d of mine.filter((x) => x.approved && x.ctx)) {
    const k = d.ctx.destination.toLowerCase();
    destCounts[k] = (destCounts[k] ?? 0) + 1;
  }
  const profile = {
    approvedCount: mine.filter((d) => d.approved).length,
    objectedCount: mine.filter((d) => !d.approved).length,
    typicalApprovedAmountMON: approvedAmts.length ? approvedAmts[Math.floor(approvedAmts.length / 2)] : null,
    destinationSeenBefore: (destCounts[to.toLowerCase()] ?? 0) > 0,
  };
  return {
    vault,
